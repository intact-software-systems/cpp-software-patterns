...
TEST(RxTransport, ReadWriteChannel)
{
    ScopedTimer a(IINFO());
...
TEST(RxTransport, PublisherSubscriber)
{
    ScopedTimer a(IINFO());
    ...
    std::shared_ptr<SubscriberChannel<std::string>> subscriber = ChannelFactory::CreateSubscriber<std::string>(handle);

    setSubscriberCallback(subscriber);

    std::shared_ptr<PublisherChannel<std::string>> publisher = ChannelFactory::CreatePublisher<std::string>(handle);
